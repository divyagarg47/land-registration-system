...
function getResponsePayload(proposalResponse) {
    ...
    // Collect all response messages from the peers
    const messages = proposalResponse.responses.map(response => response.response?.message || ""); 

    // Join all messages into a single string (or structure as needed)
    const combinedMessage = messages.join("|");  // Example: "yes; no; yes"

    // Get the first valid endorsement response
    const validEndorsementResponse = getValidEndorsementResponse(proposalResponse.responses);
    if (!validEndorsementResponse) {
        const error = newEndorsementError(proposalResponse);
        logger.error('%s', error);
    ...
    let payload = (0, gatewayutils_1.getTransactionResponse)(validEndorsementResponse).payload;

    // Convert payload to JSON and append the messages
    let payloadObject;
    try {
        payloadObject = JSON.parse(payload.toString()); // Convert Buffer to JSON
    } catch (e) {
        payloadObject = {}; // If parsing fails, use empty object
    }

    // Append the messages
    payloadObject.endorsementMessages = combinedMessage;

    // console.log("Valid Endorsement Response:", validEndorsementResponse);  // Debugging
    
    const payloadString = JSON.stringify(payloadObject);

    if (typeof payloadString !== 'string') {
        logger.error("Generated payloadString is not a valid string:", payloadString);
        throw new Error("Generated payloadString is invalid.");
    }

    const resultBuffer = Buffer.from(payloadString);

    // Check if the resultBuffer is valid
    if (!resultBuffer) {
        logger.error("Resulting Buffer is undefined or null.");
        throw new Error("Failed to convert payloadObject to Buffer.");
    }
        ...
        try {
            ...
            try {
                await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Event timeout")), 20000)) // 10 seconds timeout
                ]);
